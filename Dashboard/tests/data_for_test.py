...
for i, (j, k) in enumerate(rng.uniform(1, 60, (len(F_flights), 2))):
    ...
    _flight['scheduled_time'] += td(minutes=j, seconds=k)
    _flight['actual_time'] = _flight['scheduled_time'] + td(minutes=rng.triangular(-2, 0, 3))
    ...
    for _c in _containers:
        T_tasks.append({
            ...
            'driver': None,
            'containers': _c,
            'source': _flight['flight_num'] if _flight['type_'] == 'A' else _flight['terminal'] + ' HOT',
            'destination': _flight['terminal'] + ' HOT' if _flight['type_'] == 'A' else _flight['flight_num'],
            'bay': _flight['bay'],
            'task_start_time': None,
            'task_time_taken': None
        })
